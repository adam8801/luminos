...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void fetchData2() async {
    try{
      ...
      var response = await networkHandler.post("/blog/getMyPost/0", body);
  ...
  void fetchData3() async {
    try{
      ...
      var response = await networkHandler.post("/blog/getMyPost/${val.toString()}", body);
              ...
              child: Stack(
                children: [
                  ClipRRect(
                      ...
                      child: Image.network(NetworkHandler().getImageUrlPr(tagname)+"?v=${Random().nextInt(100)}",
                        fit: BoxFit.fill,
              ...
              child: Row(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: FlatButton(
                      onPressed: (){
                        showAlertDialog(context, profileModel.tagname);
                      },
                      child: Text("Tagname", style: TextStyle(color: Colors.white ,fontSize: 15, fontWeight: FontWeight.bold),),
                    ),
                  ),
                  SizedBox(width: 10),
                  profileModel.freeze == 0 ? Center(
                    child: Container(
                      ...
                      child: FlatButton(
                        onPressed: (){
                          Navigator.push(context, MaterialPageRoute(builder: (context) => ShowSkills(tagname: profileModel.tagname, name: profileModel.name, others: false,)));
                        },
  ...
  Widget popUp1(BuildContext context, String id, String writer, List<String> reports) {
    return PopupMenuButton<String>(
      onSelected: (value) async {
          ...
          var response = await networkHandler.post("/blog/delete/pe", body);
          if(response.statusCode == 200 || response.statusCode == 201) {
            showAlertDialog(context, "Done");
            fetchData();
          }else{
