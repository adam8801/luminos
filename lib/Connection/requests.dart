...
class RequestsConnectionList extends StatefulWidget {
  @override
  _RequestsConnectionListState createState() => _RequestsConnectionListState();
}

class _RequestsConnectionListState extends State<RequestsConnectionList> {
  ...
  void fetchData() async {
    try{
      var response = await networkHandler.get("/profile/getRequests");
      Map<String, dynamic> body ={
        "list" : response["data"]["rconnections"]
      };
        ...
        circularState ? CircularProgressIndicator() : Container(
          ...
          child: data != [] ?  ListView.builder(
            ...
            itemBuilder: (context, index) => InkWell(
                ...
                child: pageTile(data[index].tagname,data[index].name, data[index].status ?? "" , data[index].level)
            ),
          ) : Container(),
        )
      ],
  ...
  Widget pageTile(String tagname, String title, String members, int auth) {
    return Container(
            ...
            child: InkWell(
              onTap: () async {
                try{
                  ...
                  var response = await networkHandler.patch("/profile/connect", body);
              ...
              child: Center(child: Text("Accept", style: TextStyle(fontWeight: FontWeight.bold)))
            )
          )
        ],
      ),
